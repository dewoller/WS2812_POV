...
int a1 = 0;     // potentiometer wiper (middle terminal) connected to analog pin 3
int a2 = 1;   

                       // outside leads to ground and +5V

int v1 = 0;           // variable to store the value read
int v2 = 0;           // variable to store the value read
int LEDscaleFactor = (int) (1024 / LED_COUNT);
int colorScaleFactor = (int) (1024 / 192);
...

{
  ...
  v1 = analogRead(a1);    // read the input pin
  v2 = analogRead(a2);    // read the input pin

  //Serial.println(v1);             // debug value
  //Serial.println(v2);             // debug value
  //Serial.println("______");             // debug value
  lightOne( rainbowOrder(int(v2 / colorScaleFactor )), (int) (v1/LEDscaleFactor));
delay(1);
}
...
void lightOne(unsigned long color, unsigned long index)
{
      clearLEDs();  // Turn off all LEDs
    for (int j=1; j<5; j++)
    {
      if (index-j >= 0)
		leds.setPixelColor(index-j, color);  
      if (index-j <= LED_COUNT)
        leds.setPixelColor(index-j, color);
      }
      leds.show();
}
